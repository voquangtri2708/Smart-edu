    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3 d-flex justify-content-between align-items-center flex-wrap">
        <h5 class="m-0 font-weight-bold text-primary">
          <i class="bi bi-calendar-week me-2"></i>Lịch Dạy Giáo Viên
        </h5>
      ...
      <div class="card-body">
        ...
        <div v-if="loading" class="text-center my-5">
          ...
          <p class="mt-2">Đang tải dữ liệu lịch dạy...</p>
        </div>
...
const setupResizeObserver = () => {
  ...
  resizeObserver = new ResizeObserver(entries => {
    for (const entry of entries) {
      ...
      // Check if size actually changed significantly (more than 3px)
...
const handleDatesSet = (info) => {
  ...
  // Chỉ tải dữ liệu khi date range thực sự thay đổi
  const newStartDate = start.toISOString().slice(0, 10);
  const newEndDate = end.toISOString().slice(0, 10);
  
  // So sánh với date range trước đó
  if (
    (newStartDate !== lastStartDate.value || 
     newEndDate !== lastEndDate.value) && 
    !loading.value
  ) {
    if (datesSetTimeout) {
      clearTimeout(datesSetTimeout);
    }
    
    // Đặt delay nhỏ để tránh nhiều lần gọi khi calendar đang khởi tạo
    datesSetTimeout = setTimeout(() => {
      lastStartDate.value = newStartDate;
      lastEndDate.value = newEndDate;
      
      // Chỉ fetch dữ liệu nếu không phải lần đầu load hoặc là lần đầu load
  ...
  // Update size when dates are set
...
const fetchSchedules = async () => {
  try {
    ...
    if (!token) {
      error.value = 'Bạn cần đăng nhập lại để xem lịch dạy.';
      loading.value = false;
      return;
    }
    ...
    error.value = 'Không thể tải lịch dạy. Vui lòng thử lại sau.';
    scheduleData.value = [];
    calendarEvents.value = [];
  } finally {
    loading.value = false;
...
const fetchClasses = async () => {
  try {
    ...
    if (!token || !teacherId) {
      console.error('Missing auth token or teacher ID');
      return;
    }
    
    // Use the actual API endpoint with auth token
    const response = await axios.get(`http://localhost:5000/api/class_teachers/teacher/${teacherId}/classes`, {
      headers: {
        'Authorization': token
      }
    });
